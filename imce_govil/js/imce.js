...
fopValidate: function(fop) {
  ...
  switch (fop) {
    case 'delete':
      ...
      jQuery("#file-list tr.selected").each(function(){
        file = imce.fileGet(this.id);
        jQuery.ajax({
          url: Drupal.settings.basePath + "imce_file_usage_list",
          type: "post",
          data: { "relpath": file.relpath, "langcode": langcode },
          async: false,
          success:function(data) { mes += data; }
        });
      }); 
      if(mes.length > 0){
        var dialogDiv = $(document.createElement('div')); 
        mes = '<div class="imce-title">' + Drupal.t("The following files will be deleted from the following pages (and page versions)")+ ":</div>" + mes;
        dialogDiv.html(mes);
        dialogDiv.dialog({
          width: 'auto',
          buttons: [
            {
              ...
              click: function() {
                $(this).dialog("close");
                if(confirm(Drupal.t('Are you sure you want to delete the selected files?'))){
                  jQuery("#file-list tr.selected").each(function(){
                    file = imce.fileGet(this.id);
                    jQuery.ajax({
                      url: Drupal.settings.basePath + "imce_file_usage_delete",
                      type: "post",
                      data: { "relpath": file.relpath },
                      async: false,
                    });
                  });
                  Drupal.settings.after_usage_delete = true;
                  jQuery('#op-item-delete a').trigger('click');
                  return true;
                }
              }
            },
        ...
        if(Drupal.settings.after_usage_delete === true){ 
          Drupal.settings.after_usage_delete = false;
          return true;
        }
        else{
          return confirm(Drupal.t('Are you sure you want to delete the selected files?'));
        }
      } 
    case 'thumb':
