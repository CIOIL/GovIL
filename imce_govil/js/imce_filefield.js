...
Drupal.behaviors.imce_filefield = {attach: function(context, settings) {
  ...
  $.each(set.fields, function(fieldID, conf) {
    ...
    $opener.text(imce_filefield.openerText()).click(function() {
      var og_group_ref = getUrlParameter('og_group_ref');
      var tnid = getUrlParameter('translation');
      var form_build_id = $('input[name="form_build_id"]').val();
      if (og_group_ref === undefined){
        og_group_ref = '';
      }
      else{
      ...
      window.open(set.url + '/' + conf.path + (set.url.indexOf('?') < 0 ? '?' : '&') + 'app=imce_filefield|imceload@imce_filefield.imceload&fieldID=' + fieldID + '&tnid=' + tnid + '&formid=' + form_build_id + '&parent_url=' + window.location.pathname + og_group_ref , '', 'width=760,height=560,resizable=1');
      return false;
    });
